...
public class EmployeeControllerTest {
    ...
    /*
     * @Test public void testDeleteEmployee(Integer id) throws Exception { Employee e1 = new
     * Employee(); e1.setEmpid(1);
     * 
     * 
     * // when(employeeService.deleteEmployee(1)).thenReturn(e1);
     * 
     * MvcResult res = mockMvc.perform(get("/deleteEmployee").accept(MediaType.APPLICATION_JSON))
     * .andExpect(status().isOk()) .andReturn(); Employee e = new
     * ObjectMapper().readValue(res.getResponse() .getContentAsString(), Employee.class);
     * assertEquals(e.getEmpid(), 1); }
     * 
     * @Test public void testsaveEmployee(Integer id) throws Exception { Employee e1 = new
     * Employee(); e1.setEmpid(1);
     * 
     * 
     * // when(employeeService.deleteEmployee(1)).thenReturn(e1);
     * 
     * MvcResult res = mockMvc.perform(get("/saveEmployee").accept(MediaType.APPLICATION_JSON))
     * .andExpect(status().isOk()) .andReturn(); Employee e = new
     * ObjectMapper().readValue(res.getResponse() .getContentAsString(), Employee.class);
     * assertEquals(e.getEmpid(), 1);
     * 
